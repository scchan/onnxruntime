...
OpKernelInfo::OpKernelInfo(const onnxruntime::Node& node,
                           ...
                           int device_id,
                           const IAllocatorManager& allocs,
...
OpKernelInfo::OpKernelInfo(const OpKernelInfo& other)
    : OpKernelInfo(other.node_, other.kernel_def_, other.device_id_, other.allocs_, other.constant_initialized_tensors_,
...
AllocatorPtr OpKernelInfo::GetAllocator(int device_id, OrtMemType mem_type) const {
  return allocs_.GetAllocator(device_id, mem_type);
}
